...
public class UserDirectionsCommand implements ApplicationCommand {
	...
	public void run(final Game game, final MessageOutput messageOutput, final MessageInput messageInput) throws IOException {
		...
		try {
			game.move(x, y);
			if (game.isCompleted()) {
				messageOutput.show("msg_game_completed", game.getWinner().toString());
			} else {
				messageOutput.show("msg_next_player_move", game.getBoard().toString(), game.getActivePlayer().toString());
